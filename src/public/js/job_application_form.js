...
function checkYear(year) {
  const regDate = /[1-9][0-9][0-9]{2}/;
  return regDate.test(year);
}
...
function checkBasicDetails() {
  ...
  if (checkPhoneNumber("contactNo") == false) {
    flag = false
  }
...
function checkEducationDetails() {
  ...
  passingyear.forEach((element) => {
    var ele = document.getElementById(element);
    if (ele.value != "") {
      if (checkYear(ele.value) == false) {
...
function checkWorkExperienceDetails() {
  var flage = true;
  var fromDate = ["fromDate1", "fromDate2", "fromDate3"];
  var toDate = ["toDate1", "toDate2", "toDate3"];
  if (
    groupCheck("companyName1", ["designation1", "fromDate1", "toDate1"]) ==
      false ||
    groupCheck("companyName2", ["designation2", "fromDate2", "toDate2"]) ==
      false ||
    groupCheck("companyName3", ["designation3", "fromDate3", "toDate3"]) ==
      false
  ) {
    return false;
  }
  fromDate.forEach((element) => {
    var ele = document.getElementById(element);
    if (ele.value != "") {
      if (checkDateFormat(ele.value) == false) {
        if (document.getElementById("span" + element) == null) {
          createDiv(element);
        }
        flage = false;
...
function checkRefrences() {
  var flag = true;
  var contacts = ["contactNumber1", "contactNumber2", "contactNumber3"];
  if (
  ...
  contacts.forEach((element) => {
    var ele = document.getElementById(element);
    if (ele.value != "") {
      if (validatePhone(ele.value) == false) {
        if (document.getElementById("span" + element) == null) {
          createDiv(element);
        }
        flag = false;
  ...
  educationdetails.forEach((ele) => {
    Object.keys(ele).forEach(function (prop) {
      if (prop == "board") {
        ...
        document.getElementById(prop + i).setAttribute("value", ele[prop]);
      }
    });
    i++;
  });
  ...
  workexpdetails.forEach((ele) => {
    ...
    Object.keys(ele).forEach(function (prop) {
      if (q == 3 || q == 4) {
        ...
        document.getElementById(prop + j).setAttribute("value", date[0]);
      } else {
...
function populatePrefrences(prefrencesdetails) {
  Object.keys(prefrencesdetails).forEach(function (prop) {
    if (prop == "location" && prefrencesdetails[prop] != null) {
      ...
      document.getElementById("department").value = prefrencesdetails[prop];
    } else {
...
function getData(id) {
  fetch("http://localhost:3000/api/jobapplication/getdata?id=" + id)
    ...
    .then((data) => {
      ...
      populatePrefrences(prefrencesdetails[0]);
    })
    .catch((error) => {
